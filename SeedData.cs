...
namespace IndyBooks
{
    public static class SeedData
    {
        ...
        public static async Task Seed(IndyBooksDataContext context)
        {
            ...
            string[] authorFirst = new string[authors.Length];
            string[] authorLast = new string[authors.Length];

            for (int i=0; i<authors.Length; i++)
            {
                authorFirst[i] = authors[i].Split(" ", 2)[0];
                
                authorLast[i] = authors[i].Split(" ", 2)[1];
            }
           ...
           //*
            ...
            var testWriters = new Faker<Writer>()
                .RuleFor(w => w.FirstName, f => f.PickRandom(authorFirst))
                .RuleFor(w => w.LastName, f => f.PickRandom(authorLast));
